...
//Aquí voy a leer la información creo una constante listados de trabajos
const jobsListings = [
  {
    id: 1,
    company: "Photosnap",
...
//Función para llamar HTML y devolverá el html de la etiqueta
...
function getJobListingHTML(jobData, filterTags = []) {
  ...
  //accumulator, current actual current value valor actual, reducir toma el valor del acumulador  y el valor actual, el 2do parametro es el valor inicial que es una cadena, la funcion que reduce el string se obtiene del tag
  const tagsString = tagsList.reduce((acc, currentTag) => {
    const activeClass =
      (filterTags.includes(currentTag) && TAG_ACTIVE_CLASS) || "";
    //acumular el valor anterior  mas la etiqueta html actual
    return acc + getTagHTML(currentTag, `${TAG_CLASS} ${activeClass}`);
  }, "");

  return jobListingHTML.replace(JOB_TAGS_PLACEHOLDER, tagsString);
}
//otra funcion separada el = element
...
function getSearchBarTags(tagValue, searchContentEl) {
  let searchBarTags = Array.from(searchContentEl.children)
    ...
    .filter((tag) => !!tag);
  //cuando la busqueda de barra de etiquetas incluye el valor de la etiqueta tagValue, lo va a remover...  estoy
  if (searchBarTags.includes(tagValue)) {
    searchBarTags = searchBarTags.filter((tag) => tag !== tagValue);
    //...si no lo va a agregar
  } else {
...
function setJobsListings(filterTags) {
  //const listado de trabajos html = listados de trabajos reducidos con el acumulado y listados actuales
  const jobsListingsHTML = jobsListings.reduce((acc, currentListing) => {
    //retornamos listado actual + listado de trabajos html con el parámetro de los listados actuales
    return acc + getJobListingHTML(currentListing, filterTags);
    //string vacio con el acumulado  del valor obtenido del html que liste los trabajos actuales
  }, "");
  //Para obtener todas las ofetas de trabajos
...
//Ahora activarlo en la barra de búsqueda y escuchar todos los eventos en la ventana
window.addEventListener("click", (event) => {
  ...
  if (targetEl.id === "clear" || !searchBarTags.length) {
    ...
    //Si no contiene esa clase, regresa
    return;
  }
